...
from py_ui.series_show import Ui_show
...
class ShowSeriesWindow(QDialog):
    # Emit meaningful payloads so parent can update UI without reloading everything
    series_deleted = Signal(int)                 # series id
    series_moved = Signal(int, str)              # series id, new_section
    series_updated = Signal(object)              # series object

    def __init__(self, section: str, series_id: int, parent=None):
        ...
        self.id = series_id
        self.series: Optional[Series] = None
        ...
        self._load_series()   # populates self.series and refreshes UI
    ...
    def _connect_signals(self):
        # Main actions
        self.ui.show_delete_button.clicked.connect(self.delete_current_series)
    ...
    def _load_series(self):
        """Load series from DB and refresh UI. Wrap DB calls with error handling."""
        try:
            self.series = get_series_by_id(self.id)
        except Exception as e:
            logger.exception("Failed to fetch series by id %s: %s", self.id, e)
            QMessageBox.critical(self, "Error", f"Failed to load series: {e}")
            self.close()
            return

        if not self.series:
            QMessageBox.critical(self, "Error", "series not found.")
            self.close()
            return

        # Cache original image URL for restore
        self.original_image_url = self.series.poster_path or ""

        self.refresh_display()

    def refresh_display(self):
        """Populate the view UI with current series data from self.series"""
        series = self.series
        if not series:
            return

        self.ui.show_name_lable.setText(series.title or "No title")
        self.ui.show_time_lable.setText(f"{series.runtime} min" if series.runtime else "Runtime not available")
        self.ui.show_label.setText(str(series.year) if series.year else "Year not available")
        self.ui.show_gener_lable.setText(", ".join(series.genres) if series.genres else "No genres")
        self.ui.show_imdb_rate_lable.setText(str(series.imdb_rating) if series.imdb_rating else "No rating")
        self.ui.show_user_rate_lable.setText(str(series.user_rating) if series.user_rating else "No user rating")
        self.ui.imdb_votes.setText(str(series.imdb_votes) if series.imdb_votes else "No votes")
        self.ui.tmdb_rate.setText(str(series.tmdb_rating) if series.tmdb_rating else "No rating")
        self.ui.tmdb_votes.setText(str(series.tmdb_votes) if series.tmdb_votes else "No votes")
        self.ui.rotten_tomatos_rate.setText(str(series.rotten_tomatoes) if series.rotten_tomatoes else "No  rating")
        self.ui.metascore_rate.setText(str(series.metascore) if series.metascore else "No score")
        self.display_cast(self.ui.cast_layout,series.cast,self._load_image_safe)

        if series.creator:
            name, url = series.creator.split(",", 1)
            self._load_image_safe(url, self.ui.director_label)
            self.ui.director_name.setText(str(name) if name else "Creator Name Unknown")


        self.add_seasons(series.seasons)
        ...
        if series.poster_path:
            self._load_image_safe(series.poster_path, self.ui.show_image_lable)
        else:
    ...
    def enter_edit_mode(self):
        ...
        if not self.series:
            QMessageBox.warning(self, "No series", "series not loaded.")
            return
    ...
    def apply_edit(self):
        ...
        if not self.series:
            QMessageBox.critical(self, "Error", "series not loaded.")
            return
        ...
        try:
            ...
            self.series = m
            self.refresh_display()
            self.series_updated.emit(m)
            QMessageBox.information(self, "Success", "series updated successfully.")
            self._exit_edit_mode()
        except Exception as e:
            logger.exception("Failed to update series %s: %s", self.id, e)
            QMessageBox.critical(self, "Error", f"Failed to update series: {e}")

    # ---------------- Delete & Move ----------------
    def delete_current_series(self):
        reply = QMessageBox.question(self, "Confirm Delete", "Are you sure you want to delete this series?",
                                     QMessageBox.Yes | QMessageBox.No)
        if reply != QMessageBox.Yes:
            return

        try:
            success = delete_series(self.id)
            if success:
                self.series_deleted.emit(self.id)
                self.close()
                QMessageBox.information(self, "Deleted", "series deleted successfully.")
                
            else:
                QMessageBox.warning(self, "Error", "Failed to delete series.")
        except Exception as e:
            logger.exception("Failed to delete series %s: %s", self.id, e)
            QMessageBox.critical(self, "Error", f"Failed to delete series: {e}")
    ...
    def move_to(self):
        ...
        try:
            success = move_series_section(self.id, new_section)
            if success:
                self.series_moved.emit(self.id, new_section)
                QMessageBox.information(self, "Series", f"series moved to {new_section}.")
                self.close()
            else:
                QMessageBox.warning(self, "Error", "Failed to move series.")
        except Exception as e:
            logger.exception("Failed to move series %s to %s: %s", self.id, new_section, e)
            QMessageBox.critical(self, "Error", f"Failed to move series: {e}")

    # ---------------- Plot / Trailer / Watch links ----------------
    def _show_full_plot(self):
        if not self.series or not self.series.plot:
            QMessageBox.information(self, "Plot", "No description available.")
            return
        QMessageBox.information(self, "series Plot", self.series.plot)

    def _open_trailer(self):
        if not self.series or not self.series.trailer:
            QMessageBox.information(self, "Trailer", "No trailer available.")
            return
        import webbrowser
        webbrowser.open(self.series.trailer)
    ...
    def add_seasons(self, seasons: Optional[list[Dict[str, Any]]]):
        if not seasons:
            return
